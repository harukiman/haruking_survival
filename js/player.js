// player.js — 状態・移動・衝突・採掘/設置・ツールtier
...
Game.Player = (function () {
  ...
  function mineTick() {
    ...
    if (mining.progress >= meta.hp) {
      breakBlock(t.tx, t.ty, obj, meta);
      mining.active = false; mining.progress = 0;
    }
  }
  ...
  function tryPlace() {
    ...
    if (!def || def.place === undefined) return;
    const t = targetTile();
    if (!t || !t.inReach || t.obj !== Game.OBJ.NONE) return;
    // solidブロックはプレイヤーの足元には置けない
    const pt = playerTile();
  ...
  function updateDrops() {
    ...
    for (let i = drops.length - 1; i >= 0; i--) {
      ...
      if (dist < PR * 2.2) { d.x += dx * 0.18; d.y += dy * 0.18; } // 吸い寄せ
      if (dist < 16) {
  ...
  return { makeDefault, spawnAt, update, targetTile, mining, playerTile, breakBlock };
})();
